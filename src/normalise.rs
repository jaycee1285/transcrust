...
///
/// Card: <https://huggingface.co/superwhisper/s1-mini> (GGUF builds at
/// `superwhisper/s1-mini-GGUF`). Three things there are load-bearing and all
/// three fail quietly, so they are recorded here rather than in a vendored copy:
///
/// - The system prompt and the control line are both part of the trained input
///   format. Drop either, reword this string, or send a value outside the
///   trained sets, and the model can hallucinate or garble instead of erroring.
/// - Thinking must be off. The chat template is Qwen3's unchanged and Qwen3
///   defaults it on, but s1-mini saw no reasoning traces in training; leave the
///   flag out and it emits an empty `<think>` block and stops, returning
///   nothing. That is the single most common way to get a blank result.
/// - Recommended input is ~1,000 tokens, which is why longer dictations are
///   chunked upstream rather than sent whole.
///
/// `prompt_matches_the_documented_template` pins the first two.
